...
def test_get_dashboard_with_attendance():
    ...
    if success:
        ...
        print_result('GET /driver-dashboard', all_elements, 
                    f'Attendance Section: {has_attendance_section}, Stats: {has_stat_cards}, ' +
...
def test_attendance_data_format():
    """Test that attendance data is in correct format for frontend"""
    ...
    if success:
        ...
        # Look for attendance_history and attendance_today in the HTML
        has_attendance_history = 'attendance_history' in response.text
        has_attendance_today = 'attendance_today' in response.text
        has_jinja_tojson = '| tojson' in response.text
        
        print_result('Attendance History in Template', has_attendance_history)
        print_result('Attendance Today in Template', has_attendance_today)
        print_result('JSON Filter in Template', has_jinja_tojson)
        
        return has_attendance_history and has_attendance_today
    else:
        print_result('Dashboard Response', False, f'Status: {response.status_code}')
        return False
...
def test_invalid_leave_reason():
    ...
    success = response.status_code in [400, 200]  # Should fail validation
    ...
    if success:
        ...
        print_result('Reject Short Leave Reason', False, f'Status: {response.status_code}')
        return False
...
def main():
    ...
    if test_driver_login():
        ...
        results.append(('Attendance Data Format', test_attendance_data_format()))
        
        # Step 4: Test marking attendance
        results.append(('Mark Present', test_mark_attendance_present()))
        
        # Step 5: Test leave request validation
        results.append(('Leave Validation', test_invalid_leave_reason()))
        
        # Step 6: Test valid leave request
